...
type DraftPick = { player: Player; team: number; round: number; overall: number };

const seededNoise = (seed: number, team: number, pick: number, playerId: string) => {
  let hash = seed ^ (team * 374761393) ^ (pick * 668265263);
  for (const character of playerId) hash = Math.imul(hash ^ character.charCodeAt(0), 2246822519);
  hash = Math.imul(hash ^ (hash >>> 13), 3266489917);
  return ((hash ^ (hash >>> 16)) >>> 0) / 4294967295;
};

function chooseComputerPlayer(available: Player[], picks: DraftPick[], team: number, overall: number, seed: number, teams: number) {
  const round = Math.floor((overall - 1) / teams) + 1;
  const roster = picks.filter((pick) => pick.team === team);
  const count = (position: string) => roster.filter((pick) => pick.player.position === position).length;
  const starters = { QB: 1, RB: 2, WR: 2, TE: 1, K: 1, DST: 1 } as Record<string, number>;
  const caps = { QB: 2, RB: 6, WR: 7, TE: 3, K: 1, DST: 1 } as Record<string, number>;
  const personality = seededNoise(seed, team, 0, "personality");

  const candidates = available.slice(0, Math.min(65, available.length)).map((player) => {
    const positionCount = count(player.position);
    let score = -player.rank * 1.12;
    if (positionCount < (starters[player.position] ?? 0)) score += round * 2.7;
    if (positionCount >= (caps[player.position] ?? 99)) score -= 120;
    if (player.position === "RB") score += (0.5 - personality) * 10;
    if (player.position === "WR") score += (personality - 0.5) * 10;
    if (player.position === "QB" && round <= 3) score -= 15 - personality * 12;
    if (player.position === "TE" && round <= 2 && player.positionRank > 3) score -= 13;
    if ((player.position === "K" || player.position === "DST") && round < 13) score -= 95;
    if (round >= 14 && positionCount === 0 && (player.position === "K" || player.position === "DST")) score += 75;
    const sameBye = roster.filter((pick) => pick.player.bye === player.bye).length;
    if (sameBye >= 3) score -= sameBye * 2;
    score += (seededNoise(seed, team, overall, player.id) - 0.5) * (12 + round * 0.35);
    return { player, score };
  }).sort((a, b) => b.score - a.score);

  // Elite opponents still occasionally disagree: choose from the tight top cluster,
  // heavily favoring the best score while producing different rooms each mock.
  const shortlist = candidates.filter((candidate) => candidate.score >= candidates[0].score - 7).slice(0, 6);
  const roll = seededNoise(seed, team, overall, "selection");
  const index = roll < 0.58 ? 0 : roll < 0.80 ? 1 : roll < 0.92 ? 2 : Math.min(shortlist.length - 1, 3 + Math.floor(roll * 3));
  return (shortlist[index] ?? candidates[0]).player;
}
...
export default function Rankings() {
  ...
  const draftSeed = useRef(Date.now());
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const computerTeam = teamOnClock(currentPick);
      const player = chooseComputerPlayer(available, picks, computerTeam, currentPick + 1, draftSeed.current, teamCount);
  ...
  }, [screen, currentPick, teamCount, draftFormat, userSeat, available, picks]);

  const startDraft = () => {
    draftSeed.current = Date.now() ^ Math.floor(Math.random() * 0x7fffffff);
  ...
  if (screen === "setup") return <main>{appHeader}<section className="mockSetup">
    <div className="setupIntro"><p className="eyebrow">MOCK DRAFT LAB</p><h1>Build your draft room.</h1><p>Choose the league shape. Weâ€™ll run a full 15-round Yahoo-style snake draft against ranking-driven opponents.</p></div>
    <div className="setupCard">
      <div className="setupCardHead"><span>Draft settings</span><small>15 ROUNDS Â· {draftFormat.toUpperCase()}</small></div>
      ...
      <button className="startDraft" onClick={startDraft} disabled={loading}>{loading ? "Loading rankingsâ€¦" : "Enter draft room â†’"}</button>
    </div>
  </section></main>;
  ...
  if (screen === "draft") {
    ...
    return <main className="draftPage">{appHeader}<section className="draftStatus"><div><p className="eyebrow">ROUND {round} OF 15</p><h1>{currentPick >= teamCount * 15 ? "Draft complete" : onClock === userSeat ? "Youâ€™re on the clock." : `Team ${onClock} is pickingâ€¦`}</h1></div><div className="pickCounter"><span>OVERALL</span><strong>{Math.min(currentPick + 1, teamCount * 15)}</strong><small>OF {teamCount * 15}</small></div></section>
      <section className="draftWorkspace">
        <div className="draftBoard"><div className="draftBoardHead"><strong>Draft board</strong><span>{teamCount} teams Â· {draftFormat} Â· {scoring}</span></div><div className={`teamLabels teams${teamCount}`}>{Array.from({length:teamCount}).map((_,i)=><span key={i}>{i+1===userSeat?"YOU":`T${i+1}`}</span>)}</div><div className={`pickGrid teams${teamCount}`}>{Array.from({length:teamCount*15}).map((_,cellIndex)=>{const roundIndex=Math.floor(cellIndex/teamCount); const team=(cellIndex%teamCount)+1; const pickOffset=draftFormat==="Snake"&&roundIndex%2===1?teamCount-team:team-1; const pickIndex=roundIndex*teamCount+pickOffset; const pick=picks[pickIndex]; return <div key={cellIndex} className={`pickCell ${team===userSeat?"yourCell":""} ${pickIndex===currentPick?"onClock":""}`}><small>{roundIndex+1}.{team}</small>{pick&&<><strong>{pick.player.name.split(" ").at(-1)}</strong><span>{pick.player.position} Â· {pick.player.team}</span></>}</div>})}</div></div>
        <aside className="playerPool"><div className="poolHead"><div><strong>Available players</strong><span>{available.length} remaining</span></div><div className="miniTabs">{positions.map((p)=><button key={p} className={position===p?"selected":""} onClick={()=>setPosition(p)}>{p}</button>)}</div></div><div className="poolRows">{filteredAvailable.map((p)=><div className="poolPlayer" key={p.id}><span className="poolRank">{p.rank}</span>{p.teamLogoUrl?<img className="poolHeadshot teamLogo" src={p.teamLogoUrl} alt={`${p.team} logo`}/>:<div className="poolHeadshot fallback">{p.team||"FA"}</div>}<button className="playerInfo" onClick={()=>showPlayer(p)}><strong>{p.name}</strong><span>{p.team} Â· {p.position}{p.positionRank}</span></button><button disabled={onClock!==userSeat || currentPick>=teamCount*15} onClick={()=>draftPlayer(p)}>Draft</button></div>)}</div></aside>
      </section>
      {selectedPlayer && <div className="detailBackdrop" onMouseDown={() => setSelectedPlayer(null)}><section className="playerDetail" role="dialog" aria-modal="true" aria-label={`${selectedPlayer.name} news`} onMouseDown={(event)=>event.stopPropagation()}><button className="closeDetail" onClick={()=>setSelectedPlayer(null)} aria-label="Close">Ã—</button><div className="detailTitle">{selectedPlayer.teamLogoUrl?<img className="detailHeadshot teamLogo" src={selectedPlayer.teamLogoUrl} alt={`${selectedPlayer.team} logo`}/>:<div className="avatar">{selectedPlayer.team||"FA"}</div>}<div><p>{selectedPlayer.position}{selectedPlayer.positionRank} Â· {selectedPlayer.team}</p><h2>{selectedPlayer.name}</h2><span>Overall rank #{selectedPlayer.rank}</span></div></div>{detailLoading?<div className="detailLoading">Loading latest RotoBaller newsâ€¦</div>:playerDetail?.error?<div className="detailLoading">{playerDetail.error}</div>:playerDetail&&<div className="newsList">{playerDetail.news?.length?playerDetail.news.map((item)=><article key={item.id}><div><span>{item.source??"RotoBaller"}</span><time>{item.timeAgo??new Date(item.updated).toLocaleDateString()}</time></div><h3>{item.title}</h3><p>{item.content}</p>{item.url&&<a href={item.url} target="_blank" rel="noreferrer">Read full update â†’</a>}</article>):<div className="detailLoading">No recent RotoBaller news for this player.</div>}</div>}</section></div>}
      {draftComplete && userReport && <section className="gradeReport">
        <div className="gradeHero"><div><p className="eyebrow">POST-DRAFT REPORT</p><h2>Your draft grade</h2><p>Seat {userSeat} Â· {teamCount}-team {scoring} Â· {draftFormat}</p></div><div className="letterGrade"><strong>{userReport.grade}</strong><span>{userReport.score}/100</span></div></div>
        <div className="gradeMetrics"><div><span>Value score</span><strong>{userReport.valueTotal >= 0 ? "+" : ""}{userReport.valueTotal}</strong><small>vs. consensus rank</small></div><div><span>Roster balance</span><strong>{Math.max(0, 100-userReport.balancePenalty*8)}%</strong><small>positional coverage</small></div><div><span>Best value</span><strong>{userReport.best?.player.name ?? "â€”"}</strong><small>{userReport.best ? `Pick ${userReport.best.overall} Â· Rank ${userReport.best.player.rank}` : ""}</small></div><div><span>Biggest reach</span><strong>{userReport.reach?.player.name ?? "â€”"}</strong><small>{userReport.reach ? `Pick ${userReport.reach.overall} Â· Rank ${userReport.reach.player.rank}` : ""}</small></div></div>
        <div className="reportGrid"><div className="rosterReport"><h3>Your roster</h3>{userReport.roster.map((pick)=><div key={pick.overall}><span>{pick.round}</span><strong>{pick.player.name}</strong><small>{pick.player.position} Â· {pick.player.team}</small><em>#{pick.overall}</em></div>)}</div><div className="leagueGrades"><h3>League grades</h3>{reports.map((report,index)=><div key={report.team} className={report.team===userSeat?"isYou":""}><span>{index+1}</span><strong>{report.team===userSeat?"You":`Team ${report.team}`}</strong><small>{report.score} points</small><em>{report.grade}</em></div>)}</div></div>
      ...
      <section className="board" aria-label="Player rankings">
        <div className="controls">
          ...
          <div className="controlRight">
            <label className="search"><span>âŒ•</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search players or teams" aria-label="Search players or teams" /></label>
        ...
        <div className="rows" aria-live="polite">
          {loading ? Array.from({length: 7}).map((_, i) => <div className="playerRow skeleton" key={i} />) : visible.map((p, i) => {
            ...
            return <div key={p.id}>{newTier && <div className="tier"><span>Tier {p.tier}</span></div>}<article className="playerRow">
              ...
              <div className="player"><div className={`avatar ${p.position.toLowerCase()}`}>{p.name.split(" ").map(n => n[0]).join("").slice(0,2)}</div><div><h2>{p.name}</h2><p>{p.team} Â· {p.position}{p.positionRank}</p></div></div>
              <span className={`position ${p.position.toLowerCase()}`}>{p.position}{p.positionRank}</span>
              <span className="bye">{p.bye}</span>
              <span className={`trend ${p.trend > 0 ? "up" : p.trend < 0 ? "down" : "flat"}`}>{p.trend > 0 ? `â†‘ ${p.trend}` : p.trend < 0 ? `â†“ ${Math.abs(p.trend)}` : "â€”"}</span>
